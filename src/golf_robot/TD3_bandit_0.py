...
from rl_common_0_no_noise import *  # DO NOT CHANGE (expects Actor, QNetwork, etc.)
...
def training(
    ...
    exploration_noise_start = rl_cfg["training"]["noise_std"]  # action noise during collection
    ...
    q1_optimizer    = torch.optim.Adam(q1.parameters(), lr=critic_lr, weight_decay=1e-5)
    q2_optimizer    = torch.optim.Adam(q2.parameters(), lr=critic_lr, weight_decay=1e-5)
    ...
    for episode in range(episodes):
        ...
        if (episode) % rl_cfg["training"]["eval_interval"] == 2 and env_type == "sim":
            ...
            exploration_noise = exploration_noise_start + frac * (exploration_noise_end - exploration_noise_start)
