...
class UR10Logger:
    ...
    def remove_outliers(self,
                    ...
                    consider: tuple[str, ...] = ("q", "dq"),
                    drop_nans: bool = True) -> dict:
        """
        Remove rows where ANY considered field exceeds a simple absolute limit.
        - thresholds: per-field max-abs; scalar (same for all columns) or list per column.
        Defaults (safe, generous):
            q:    8π rad       (~25.13)
            dq:   1000 rad/s
            tcp:  [inf, inf, inf, π, π, π]       (xyz unrestricted; rx,ry,rz bounded)
            dtcp: [5, 5, 5, 10, 10, 10]          (m/s, rad/s)
        - consider: which fields to gate the mask with. Default ("q","dq") so CB2 tcp/dtcp
        (often NaN) won’t nuke your data. Use ("q","dq","tcp","dtcp") if you want all.
        - drop_nans: if True, also drop rows with NaN/Inf in considered fields or time.
        Returns a small report dict.
        """
        import numpy as np

        def _default_limits(fname: str, w: int):
            if fname == "q":
                return np.full((w,), 8.0*np.pi, dtype=float)
            if fname == "dq":
                return np.full((w,), 1000.0, dtype=float)
            if fname == "tcp":
                base = np.array([np.inf, np.inf, np.inf, np.pi, np.pi, np.pi], dtype=float)
                return base[:w]
            if fname == "dtcp":
                base = np.array([5.0, 5.0, 5.0, 10.0, 10.0, 10.0], dtype=float)
                return base[:w]
            # Unknown field: no limits by default (treat as not considered)
            return None

        with self._lock:
            t = np.asarray(self.data.get("t", []), dtype=float)
            N = t.size
            if N == 0:
                return {"kept": 0, "removed": 0, "reason": "empty log"}

            # Build base mask (time finite or not)
            mask = np.isfinite(t) if drop_nans else np.ones(N, dtype=bool)
            ...
            for name in consider:
                ...
                if not rows:
                    skipped_fields[name] = "empty"
                    continue
                A = np.asarray(rows, dtype=float)
                if A.shape[0] != N:
                    skipped_fields[name] = "length_mismatch"
                    continue

                # Optional finite check
                m = np.isfinite(A).all(axis=1) if drop_nans else np.ones(N, dtype=bool)

                # Limits for this field
                w = A.shape[1]
                lim = th.get(name, _default_limits(name, w))
                if lim is None:
                    skipped_fields[name] = "no_limits_defined"
                    continue
                lim = np.asarray(lim, dtype=float)
                if lim.size == 1:
                    lim = np.full((w,), float(lim), dtype=float)
                lim = lim[:w]

                # Apply simple absolute threshold
                m &= (np.abs(A) <= lim).all(axis=1)

                mask &= m
                used_limits[name] = lim.tolist()
                used_fields.append(name)

            kept = int(mask.sum())
            removed = int(N - kept)

            # Apply mask back to storage for ALL fields that match length N
            self.data["t"] = t[mask].tolist()
            for name in self.fields:
                rows = self.data.get(name, [])
                if rows and len(rows) == N:
                    A = np.asarray(rows, dtype=float)
                    self.data[name] = A[mask].tolist()
    ...
    def plot_comparison(self, 
                       ...
                       save=True, 
                       show=False, 
                       title_prefix="Trajectory Comparison"):
        """
        Plot measured data (from logger) vs planned data for comparison.
        
        Args:
            t_plan: Planned time array
            P_plan: Planned TCP positions (N,3)
            Q_plan: Planned joint positions (N,6)
        ...
        # Get measured data from logger
        t_meas = self.time()
        if len(t_meas) == 0:
            print("[WARN] No measured data available for comparison")
            return plot_files
        
        # Plot joint positions comparison
        if Q_plan is not None and t_plan is not None:
            Q_meas = self.as_array("q")
            ...
            for i in range(6):
                ...
                # Plot planned
                ax.plot(t_plan, Q_plan[:, i], 'b-', label='Planned', linewidth=2, alpha=0.8)
                
                # Plot measured
                ax.plot(t_meas, Q_meas[:, i], 'r-', label='Measured', linewidth=1)
                
                ax.set_ylabel(f"{joint_names[i]} [rad]")
                ax.grid(True, linestyle="--", alpha=0.5)
                ax.legend()
                
                # Add pi formatting for joint positions
                ax.yaxis.set_major_locator(MultipleLocator(np.pi/2))
                ax.yaxis.set_major_formatter(FuncFormatter(_pi_formatter))
            
            axes[-1].set_xlabel("Time [s]")
            axes[-2].set_xlabel("Time [s]")
            fig.suptitle(f"{title_prefix}: Joint Positions")
            fig.tight_layout()
            
            if save:
                ts = time.strftime("%Y%m%d_%H%M%S")
                out = os.path.join(self.log_folder, f"comparison_joints_{ts}.png")
                fig.savefig(out, dpi=300)
                plot_files.append(out)
        ...
        # Plot TCP position comparison
        if P_plan is not None and t_plan is not None:
            # Get measured TCP from FK
            mask, X_meas, Y_meas, Z_meas = self._compute_tcp_xyz_from_q()
            t_meas_tcp = t_meas[mask]
            P_meas = np.column_stack([X_meas[mask], Y_meas[mask], Z_meas[mask]])
            ...
            for i in range(3):
                ...
                # Plot planned
                ax.plot(t_plan, P_plan[:, i], 'b-', label='Planned', linewidth=2, alpha=0.8)
                
                # Plot measured
                ax.plot(t_meas_tcp, P_meas[:, i], 'r-', label='Measured', linewidth=1)
                
                ax.set_ylabel(f"TCP {labels[i]} {units[i]}")
                ax.grid(True, linestyle="--", alpha=0.5)
                ax.legend()
            
            axes[-1].set_xlabel("Time [s]")
            fig.suptitle(f"{title_prefix}: TCP Position")
            fig.tight_layout()
            
            if save:
                ts = time.strftime("%Y%m%d_%H%M%S")
                out = os.path.join(self.log_folder, f"comparison_tcp_{ts}.png")
                fig.savefig(out, dpi=300)
                plot_files.append(out)
        ...
        if P_plan is not None:
            ...
            # Plot planned path
            ax.plot3D(P_plan[:, 0], P_plan[:, 1], P_plan[:, 2], 
                     'b-', linewidth=2, alpha=0.8, label='Planned')
            ax.scatter(P_plan[0, 0], P_plan[0, 1], P_plan[0, 2], 
                      s=50, c='blue', marker='o', label='Plan Start')
            ax.scatter(P_plan[-1, 0], P_plan[-1, 1], P_plan[-1, 2], 
                      s=50, c='blue', marker='s', label='Plan End')
            
            # Plot measured path
            ax.plot3D(P_meas[:, 0], P_meas[:, 1], P_meas[:, 2], 
                     'r-', linewidth=1, label='Measured')
            ax.scatter(P_meas[0, 0], P_meas[0, 1], P_meas[0, 2], 
                      s=30, c='red', marker='o', label='Meas Start')
            ax.scatter(P_meas[-1, 0], P_meas[-1, 1], P_meas[-1, 2], 
                      s=30, c='red', marker='s', label='Meas End')
            
            ax.set_xlabel("X [m]")
            ax.set_ylabel("Y [m]")
            ax.set_zlabel("Z [m]")
            ax.set_title(f"{title_prefix}: 3D TCP Path")
            ax.legend()
            
            # Set equal aspect ratio
            all_points = np.vstack([P_plan, P_meas])
            max_range = np.array([
            ...
            if save:
                ts = time.strftime("%Y%m%d_%H%M%S")
                out = os.path.join(self.log_folder, f"comparison_3d_path_{ts}.png")
                fig.savefig(out, dpi=300)
                plot_files.append(out)
