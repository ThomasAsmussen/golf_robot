...
def compute_reward(ball_end_pos, hole_pos, in_hole, trajectory):
    """
    Single-step reward:
      - big reward if in hole
      - otherwise shaped by final distance
    """
    if in_hole:
        return 3.0
...
# ---------------------------------------------------------
# TD3-Style Training (Contextual Bandit)
# ---------------------------------------------------------
def training(rl_cfg, mujoco_cfg, project_root, continue_training=False):
    """
    TD3-style contextual bandit:

      - context = state (ball_start_obs, hole_pos_obs, discs)
      - action  = (speed, angle)
      - reward  = f(ball_end, hole, in_hole)

    Differences from plain bandit DDPG:
      - twin critics Q1, Q2
      - actor loss uses min(Q1, Q2)
      - optional delayed policy updates (policy_delay)

    Still no bootstrapping: target = reward (one-step bandit).
    """
    ...
    # TD3-specific: delayed policy updates
    policy_delay = rl_cfg["training"].get("policy_delay", 2)

    normalizer = Normalizer(size=state_dim)

    # Linear schedule for exploration noise
    ...
    if continue_training:
        actor, device = load_actor(
            model_path=project_root / "models" / "rl" / "td3" / f"td3_actor_{model_name}",
            rl_cfg=rl_cfg,
        )
        critic1, _ = load_critic(
            model_path=project_root / "models" / "rl" / "td3" / f"td3_critic1_{model_name}",
            rl_cfg=rl_cfg,
        )
        critic2, _ = load_critic(
            model_path=project_root / "models" / "rl" / "td3" / f"td3_critic2_{model_name}",
            rl_cfg=rl_cfg,
        )
    else:
    ...
    if use_wandb:
        ...
        wandb.watch(critic1, log="gradients", log_freq=100)
        wandb.watch(critic2, log="gradients", log_freq=100)
        run_name = wandb.run.name.replace("-", "_")
    else:
        run_name = "local_run"

    model_dir = project_root / "models" / "rl" / "td3"
    ...
    max_num_discs = 0   # change if you want discs during training

    total_it = 0  # TD3-style update counter

    for episode in range(episodes):
        # -------------------------------------------------
        # Sample context (ball, hole, discs)
        # -------------------------------------------------
        ...
        critic_loss_value1 = None
        critic_loss_value2 = None
        actor_loss_value   = None

        # -------------------------------------------------
        # Policy: a = π(s) + exploration noise
        # -------------------------------------------------
        with torch.no_grad():
        ...
        if len(replay_buffer.data) >= batch_size:
            for _ in range(grad_steps):
                ...
                # Bandit: target is just reward (no bootstrapping)
                td_target = rewards_b

                # Critic losses (two critics, both regression to reward)
                q1_pred = critic1(states_b, actions_b)
                q2_pred = critic2(states_b, actions_b)

                critic1_loss = F.mse_loss(q1_pred, td_target)
                critic2_loss = F.mse_loss(q2_pred, td_target)
                ...
                critic_loss_value1 = critic1_loss.item()
                critic_loss_value2 = critic2_loss.item()

                # Delayed policy update
                if total_it % policy_delay == 0:
                    # Actor tries to maximize min(Q1, Q2)
                    a_for_actor = actor(states_b)
                    q1_for_actor = critic1(states_b, a_for_actor)
                    q2_for_actor = critic2(states_b, a_for_actor)
                    q_min = torch.min(q1_for_actor, q2_for_actor)
        ...
        if use_wandb:
            ...
            if critic_loss_value1 is not None:
                log_dict["critic1_loss"] = critic_loss_value1
            if critic_loss_value2 is not None:
                log_dict["critic2_loss"] = critic_loss_value2
    ...
    torch.save(critic1.state_dict(), model_dir / f"td3_critic1_{run_name}.pth")
    torch.save(critic2.state_dict(), model_dir / f"td3_critic2_{run_name}.pth")
...
# ---------------------------------------------------------
# Evaluation (greedy policy, TD3-style bandit)
# ---------------------------------------------------------
def evaluation_policy_short(
    ...
    with torch.no_grad():
        for _ in range(num_episodes):
            ...
            # Same actuator noise as training
            speed_noise     = np.random.normal(0, SPEED_NOISE_STD)
            angle_deg_noise = np.random.normal(0, ANGLE_NOISE_STD)
            speed     = np.clip(speed + speed_noise,     speed_low,  speed_high)
            angle_deg = np.clip(angle_deg + angle_deg_noise, angle_low, angle_high)
...
if __name__ == "__main__":
    ...
    if rl_cfg["training"]["use_wandb"]:
        ...
        rl_cfg["training"]["policy_delay"] = cfg.policy_delay
