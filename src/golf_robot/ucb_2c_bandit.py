...
# =========================================================
# Algorithm: Bootstrapped Ensemble UCB + CEM planning + Double Critic per head
# =========================================================
# - K heads (bootstrapped)
# - Each head has TWO critics: Q1_k, Q2_k
# - UCB action selection: plan with score(a)=mean_k(min(Q1_k,Q2_k)) + beta * std_k(min(Q1_k,Q2_k))
# - Train both critics by supervised regression to reward
# - Evaluation: use mean across heads (beta=0), wrapped as an "actor" so evaluation_policy_short() works
...
class MeanPlannerActor(nn.Module):
    ...
    Deterministic eval actor:
      - Plans using mean across heads of min(Q1,Q2) (i.e., UCB with beta=0)
...
def training(
    ...
    ucb_beta_final = float(rl_cfg["training"].get("ucb_beta_final", ucb_beta))
